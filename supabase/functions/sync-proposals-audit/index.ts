import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-webhook-secret",
};

const MAX_ROWS = 1000;

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  const secret = req.headers.get("x-webhook-secret");
  const expectedSecret = Deno.env.get("WEBHOOK_SECRET");
  if (!expectedSecret) {
    return new Response(
      JSON.stringify({ error: "Server configuration error" }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
  if (secret !== expectedSecret) {
    return new Response(
      JSON.stringify({ error: "Unauthorized" }),
      { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }

  try {
    const body = await req.json();
    const rows: unknown[] = Array.isArray(body) ? body : [body];

    if (rows.length > MAX_ROWS) {
      return new Response(
        JSON.stringify({ error: `Too many rows (max ${MAX_ROWS})` }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    const safeStr = (v: unknown, max = 500): string | null => {
      if (v == null) return null;
      return String(v).slice(0, max);
    };
    const safeNum = (v: unknown, def = 0): number => {
      const n = Number(v);
      return isNaN(n) || !isFinite(n) ? def : n;
    };

    const supabase = createClient(
      Deno.env.get("SUPABASE_URL")!,
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!
    );

    const mapped = rows.map((r: any) => ({
      monday_id: safeStr(r?.monday_id, 100),
      campaign_name: safeStr(r?.campaign_name),
      company: safeStr(r?.company, 200),
      status: safeStr(r?.status, 50) ?? "To Do",
      budget: safeNum(r?.budget),
      take_rate_pct: safeNum(r?.take_rate_pct),
      proposal_adjustments: safeNum(r?.proposal_adjustments),
      seller_name: safeStr(r?.seller_name, 200),
      csm: safeStr(r?.csm, 200),
      list_builder: safeStr(r?.list_builder, 200),
      building_proposal_start: safeStr(r?.building_proposal_start, 10),
      pending_approval_start: safeStr(r?.pending_approval_start, 10),
      days_since_pending: safeNum(r?.days_since_pending),
      risk_score: safeNum(r?.risk_score, 1),
      ai_insight: safeStr(r?.ai_insight, 2000),
      action_required: safeStr(r?.action_required, 1000),
      seller_sla: safeStr(r?.seller_sla, 10) ?? "NO",
      lb_sla: safeStr(r?.lb_sla, 10) ?? "NO",
      currency: safeStr(r?.currency, 10) ?? "USD",
      proposal_delivery_date: safeStr(r?.proposal_delivery_date, 10),
      audience_country: safeStr(r?.audience_country, 100),
      musical_genre: safeStr(r?.musical_genre, 100),
    }));

    const { data, error } = await supabase
      .from("proposals_audit")
      .upsert(mapped, { onConflict: "monday_id" })
      .select();

    if (error) {
      console.error("Upsert error:", error);
      return new Response(
        JSON.stringify({ error: "Database operation failed" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    return new Response(
      JSON.stringify({ success: true, count: rows.length, data }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err) {
    console.error("Sync error:", err);
    return new Response(
      JSON.stringify({ error: "Error processing request" }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
});
